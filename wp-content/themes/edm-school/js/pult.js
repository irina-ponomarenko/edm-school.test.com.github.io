...
$('#left .play').click(function() {
  if (leftSong != null) {
    diff = (((($('#left .volume').offset().top-7500)*-1)*0.5)/54);
...
$('#right .play').click(function() {
  if (rightSong != null) {
    diff = (((($('#right .volume').offset().top-7500)*-1)*0.5)/54);
...
$("#left .tempo").dblclick(function() {
  $('#left .tempo').animate({'top': 1}, 500, 'easeOutQuad');
  $('#left .info-tempo > strong').html('±0.00<span> %</span>');
  rateLeft = 1;    
});

$("#right .tempo").dblclick(function() {
  $('#right .tempo').animate({'top': 41}, 500, 'easeOutQuad');
...
function dragVolume(side) {
  diff = ((($(side+' .volume').offset().top-7595)*-1)*0.5)/54;
  	switch(side) {
      case '#left':
...
function crossFade() {
  currentVolLeft = (0.5+((($('#left .volume').offset().top-7595)*-1)*0.5)/54).toFixed(2);
  currentVolRight = (0.5+((($('#right .volume').offset().top-7595)*-1)*0.5)/54).toFixed(2);  
